<template>
  <div>
    ...
    <el-dialog
      ...
      width="60%"
    >
      <select-product :default-select="shopProduct.productList" @selectSubmit="setProductList" />
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    getUpdate(id) {
      productApi.getUpdate(id).then(res => {
        ...
        if (this.shopProduct.shopPack) {
          this.shopProduct.selectPackList = [this.shopProduct.shopPack]
        }
    ...
    setProductList(productList) {
      ...
      this.shopProduct.productList = productList
      this.selectProductDialog = false
    },
    // 传递套装列表 关闭套装弹窗
    setPack(pack) {
      this.shopProduct.shopPack = pack
      this.selectPackList = [pack]
      this.selectPackDialog = false
    },
    deleteProduct(id) {
      this.shopProduct.productList.splice(
        this.shopProduct.productList.findIndex(e => e.id === id),
        1
      )
    },
    categoryChange(ids) {
